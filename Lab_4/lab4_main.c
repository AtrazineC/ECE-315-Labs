...
volatile int parameters_flag = 0;

volatile int positionSequence[SEQUENCE_LENGTH][2] = {
        {NO_OF_STEPS_PER_REVOLUTION_FULL_DRIVE, 0}}; // position-delay array
volatile int sequenceIndex = 0; // the number of position-delay sequences
volatile int loop_count = 1; // the number of times to repeat the position-delay sequence
volatile long target = 0; // Current target
        ...
        if (invalid_input_flag == 1) {
            ...
            if (parameters_flag == 0) {
                ...
                xil_printf("*** Pair: %d\t, <destination, delay> = <%d, %d>\n\n", sequenceIndex + 1,
                           positionSequence[sequenceIndex][0], positionSequence[sequenceIndex][1]);
                ...
                if (keep_default_value_flag == 1 || sequenceIndex == SEQUENCE_LENGTH) {
                    ...
                    while (1) {
                        ...
                        if (XUartPs_IsReceiveData(XPAR_XUARTPS_0_BASEADDR)) {
                            command_1_or_2_values[index] = XUartPs_ReadReg(XPAR_XUARTPS_0_BASEADDR,
                                                                           XUARTPS_FIFO_OFFSET);
                            index++;
...
static void _Task_Motor(void *pvParameters) {
    ...
    while (1) {
        ...
        for (int i = 0; i < sequenceIndex; i++) {
            target = positionSequence[i][0];
            xil_printf("Test = %d\n", target);
            Stepper_moveToPositionInSteps(target);
            xil_printf("Disable\n");
...
static void _Task_Emerg_Stop(void *pvParameters) {
    ...
    while (1) {
        ...
        if (pressedCount >= 3) {
            ...
            Stepper_setCurrentPositionInSteps(target);
            sequenceIndex = 0;
            Stepper_disableMotor();
            vTaskDelete(xMotortask);
            vTaskDelete(xUarttask);

            xil_printf("motor emergency stop triggered");
            int half_time = (1000 / 2) / 2;
